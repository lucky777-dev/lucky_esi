\documentclass{standalone}

\usepackage[T1]{fontenc} %encoding packages
\usepackage{textcomp}
\usepackage[utf8x]{inputenc}
\usepackage[french]{babel} %french typesetting package

\usepackage[x11names,dvipsnames,table]{xcolor}
\usepackage{colortbl}%more colors

\usepackage{tikz}%tikz & pgf
\usepackage{tkz-fct} %fct tkz
\usepackage{pgfplots} %make sure floating points operations are made properly
\usepackage{calc}
\usepackage{pgf}
\usepackage{ifthen} %if / then / else
\usepackage{listings}

%make computations in tikz
\usetikzlibrary{calc}
\usetikzlibrary{decorations.pathreplacing}
\usetikzlibrary{patterns}
\usetikzlibrary{quotes}
\usetikzlibrary{angles}

\lstset{language=c++,		
		keywordstyle=\bf\color{blue!60},
		commentstyle=\color{Green},
%		basicstyle=\scriptsize,
		tabsize=2}

\tikzstyle{_vertex}=[fill=white, circle,minimum size=12pt,inner sep=1pt]
\tikzstyle{_blackv}=[fill=black, circle,minimum size=8pt,inner sep=1pt]
\tikzstyle{_whitev}=[fill=white, circle,minimum size=8pt,inner sep=1pt]
\tikzstyle{_dot}=[fill=black, circle, minimum size = 1.5mm, inner sep=0pt]
\tikzstyle{_wdot}=[fill=white, circle, minimum size = 1.5mm, inner sep=0pt]
\tikzstyle{_inv}=[inner sep=0pt, minimum size=0pt]

\tikzstyle{_bigvertex}=[fill=white, circle,minimum size=21pt,inner sep=1pt]
\tikzstyle{_arc}=[->, >=stealth]
\tikzstyle{_boldarc}=[->, >=stealth, line width=2pt]

\begin{document}
\begin{tikzpicture}
\draw (0,-1) -- (0,-3) -- (2,-3) -- (2,-1);
\draw (0,-2) -- (2,-2);
\filldraw[fill=blue!40] (0,0) rectangle (2,-1);
\node at (1,-0.5) {\texttt{v}};
\node at (1,-1.5) {$5$};

\draw (0,-5) -- (0,-7) -- (2,-7) -- (2,-5);
\draw (0,-6) -- (2,-6);
\filldraw[fill=blue!40] (0,-4) rectangle (2,-5);
\node at (1,-4.5) {\texttt{v'}};
\node at (1,-5.5) {$5$};

\foreach \i in {-2,...,-5}
{
	\draw (4,\i) -- (6,\i);
}
\draw (4,-1) rectangle (6,-6);
\filldraw[fill=blue!40] (4,0) rectangle (6,-1);
\node at (5,-0.5) {\texttt{tab}};
\node at (5,-1.5) {1};
\node at (5,-2.5) {3};
\node at (5,-3.5) {5};
\node at (5,-4.5) {4};
\node at (5,-5.5) {2};

\draw[->, very thick] (1,-2.5) to[out=0,in=180] (4,-0.5);
\draw[->, very thick, red, dashed] (1,-6.5) to[out=0,in=180] (4,-0.5);
\node[rotate=85] at (3, -3.5) 
{
\begin{lstlisting}
delete[] tab;
\end{lstlisting}
};

\draw[very thick, red] (-0.5,-3.5) -- (2.5,-7.5);
\draw[very thick, red] (-0.5,-7.5) -- (2.5,-3.5);
\draw[very thick, red] (3.5,0.5) -- (6.5,-6.5);
\draw[very thick, red] (3.5,-6.5) -- (6.5,0.5);
\end{tikzpicture}
\end{document}